#if 0 //To compile, type: nmake map2stl.c
map2stl.exe: map2stl.c; cl /J /TP map2stl.c /G6Fy /Gs /MD /link /opt:nowin98
   del map2stl.obj
!if 0
#endif
...
static void checknextwalls ()
{
   float x0, y0, x1, y1;
   long s0, w0, w0n, s1, w1, w1n;
...
static void shellsrt (float *a, long n)
{
   float t;
   long i, j, g;
...
static float getslopez (sect_t *s, long i, float x, float y)
{
   wall_t *wal = s->wall;
...
static long sect2trap (wall_t *wal, long n, zoid_t **retzoids, long *retnzoids)
{
   zoid_t *zoids = 0;
   ...
   for(s=0;s<secn-1;s++)
   {
      ...
      for(i=0;i<n;i++) //FIX:optimize
      {
         ...
         trapx0[ntrap] = x0; trapx1[ntrap] = x1; pwal[ntrap] = (long)&wal[i]; ntrap++;
      }
      for(g=(ntrap>>1);g;g>>=1)
         for(i=0;i<ntrap-g;i++)
            for(j=i;j>=0;j-=g)
...
static long getwalls (long s, long w, vertlist_t *ver, long maxverts)
{
   vertlist_t tver;
   wall_t *wal, *wal2;
   float fx, fy;
...
static long wallclip (kgln_t *pol, kgln_t *npol)
{
   float f, dz0, dz1;
   ...
   //   short filler;
   //}
static void saveasstl (char *filnam)
{
   ...
   FILE *fil;
   float f, fz;
   long i, j, k, n, w, s, nw, wn, vn, s0, cf0, s1, cf1, isflor, nzoids;
   long numtris;
   char tbuf[80];
      ...
      for(isflor=0;isflor<2;isflor++)
      {
         ...
         for(i=0;i<nzoids;i++)
         {
            ...
            for(j=2;j<n;j++)
            {
               ...
               f = fp2.x*fp2.x + fp2.y*fp2.y + fp2.z*fp2.z; if (f > 0.f) f = -1.f/(float)sqrt(f);
...
{
   if (argc != 3) { printf("map2stl [in:Build .MAP (v7)] [out:STL file]       by Ken Silverman (05/15/2009)"); return(1); }
   if (!loadmap(argv[1])) { printf("error loading map\n"); return(2); }
   saveasstl(argv[2]);
   return(0);
}
